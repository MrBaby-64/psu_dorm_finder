...
<div class="bg-white shadow-sm rounded-lg">
    ...
    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
        <form method="GET" action="{{ route('landlord.notifications') }}" class="flex flex-wrap gap-4">
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['filter', 'type']))
                    <a href="{{ route('landlord.notifications') }}" class="bg-gray-300 text-gray-700 px-4 py-1 rounded-lg hover:bg-gray-400 text-sm">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="divide-y divide-gray-200">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if(!$notification->is_read)
                                        <form method="POST" action="{{ route('landlord.notifications.read', $notification) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs text-gray-600 hover:text-gray-800 underline bg-transparent border-0 p-0 cursor-pointer">
                                                Mark as Read
                                            </button>
                                        </form>
                                    @endif
        ...
        @empty
            <div class="px-6 py-12 text-center">
                ...
                <p class="text-gray-600">You'll see notifications here when tenants send inquiries, request visits, leave reviews, or when there are updates about your properties.</p>
            </div>
        @endforelse
    </div>
