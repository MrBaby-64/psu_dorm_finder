                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mt-6 px-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        @php
                            ...
                            $landlordPropertyIds = $host->properties->pluck('id');

                            // Total Inquiries for all landlord's properties
                            $totalHostInquiries = \App\Models\Inquiry::whereIn('property_id', $landlordPropertyIds)->count();

                            // Total Favorites across all landlord's properties
                            $totalHostFavorites = \App\Models\Favorite::whereIn('property_id', $landlordPropertyIds)
                                ->distinct('user_id')
                                ->count('user_id');

                            // Total Approved Visits across all landlord's properties
                            $totalApprovedVisits = \App\Models\ScheduledVisit::whereIn('property_id', $landlordPropertyIds)
                                ...
                                @auth
                                    @if(auth()->user()->role === 'tenant' && auth()->id() !== $host->id)
                                        <button onclick="openReportHostModal()"
                                                class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 hover:border-red-300 transition">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="space-y-2 mt-3 text-sm">
                                        ...
                                        <div class="flex items-center gap-2 text-gray-600">
                                            ...
                                            <span>{{ $totalHostInquiries }} {{ $totalHostInquiries === 1 ? 'Inquiry' : 'Inquiries' }}</span>
                                        </div>
                                        <div class="flex items-center gap-2 text-gray-600">
                                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z"></path>
                                            </svg>
                                            <span>{{ $totalHostFavorites }} {{ $totalHostFavorites === 1 ? 'User Favorite' : 'User Favorites' }}</span>
                                        </div>
                                        <div class="flex items-center gap-2 text-gray-600">
                                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"></path>
                                            </svg>
                                            <span>{{ $totalApprovedVisits }} Approved {{ $totalApprovedVisits === 1 ? 'Visit' : 'Visits' }}</span>
                                        </div>
                                    </div>
...
<div id="reportHostModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
            ...
            <!-- Report Form -->
            <form id="reportHostForm" action="{{ route('landlord.report', $property->user_id) }}" method="POST">
                @csrf
                <input type="hidden" name="property_id" value="{{ $property->id }}">
    ...
    document.getElementById('reportHostForm')?.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                ...
                showSimpleAlert('✅ Report submitted successfully! Our team will review it and take appropriate action. Thank you for helping keep our community safe.', 'success');
            } else {
                showSimpleAlert('❌ ' + (data.message || 'Failed to submit report. Please try again.'), 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showSimpleAlert('❌ An error occurred. Please try again later.', 'error');
        });
    });
</script>
